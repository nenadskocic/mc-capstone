...
async function startup()
{
  ...
  await db.run("CREATE TABLE user_auth (auth_id INTEGER NOT NULL UNIQUE, username TEXT NOT NULL UNIQUE, hash_pwd TEXT NOT NULL, user_id INTEGER NOT NULL, PRIMARY KEY (auth_id AUTOINCREMENT), FOREIGN KEY (user_id) REFERENCES user (user_id))");
...
async function register_user() {
  try {
    ...
    await db.run("INSERT INTO user_auth (username, hash_pwd, user_id) VALUES ('" + admin_username + "', '" + hash + "', 1)");
  } catch(e) {
    console.log(e)
  }
}
